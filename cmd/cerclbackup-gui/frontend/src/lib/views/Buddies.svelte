  ...
  <div class="grid">
    <div class="card">
      ...
      {#if invite}
        ...
        <p><strong>{$t('buddies.invite.joinAddr')}:</strong> {invite.JoinAddr} <button on:click={() => copy(invite!.JoinAddr)}>{$t('common.copy')}</button></p>
      {/if}
    </div>
...
<style>
  ...
  .card { background: rgba(255,255,255,0.06); border-radius: 8px; padding: 1rem; text-align: left; display: flex; flex-direction: column; gap: 0.6rem; }
